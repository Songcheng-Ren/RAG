...
def process_chunks_and_generate_relations_incremental(
    ...
    print(f"\n增量处理（每处理 {save_interval} 个chunk保存一次）...")
    print(f"Chunks总数: {len(chunks)}, 批次大小: {batch_size}")
    ...
    def process_chunk_entities_with_llm(chunk_entities: List[str], context: str):
        if not chunk_entities or len(chunk_entities) < 2:
            return set()
        ents_text = "\n".join([f"- {e}" for e in chunk_entities])
        prompt = (
            "You are a knowledge graph expert specializing in hierarchical (parent-child) relationships.\n\n"
            "Entities provided (hints):\n"
            f"{ents_text}\n\n"
            "Context:\n"
            f"{context[:800]}\n\n"
            "Task:\n"
            "1) First, identify all meaningful entities that appear in the context text. Use the provided list as hints, but also include any other entities present in the context (e.g., people, organizations, locations, technical terms, concepts, events).\n"
            "   IMPORTANT: The provided entities must remain unchanged. Do NOT modify, normalize, merge, split, or rename them.\n"
            "2) Next, determine parent-child hierarchical relationships among the entities that appear in the context.\n"
            "   - Parent: a broader/general class/category.\n"
            "   - Child: a more specific/member/subtype under the parent.\n"
            '   - Examples: "动物" (parent) and "狗" (child); "编程语言" (parent) and "Python" (child).\n'
            "3) Only output relationships supported by the context. Co-occurrence without hierarchy must NOT be output.\n\n"
            "Output format requirements:\n"
            "- One relationship per line, format: child,parent\n"
            "- child is the more specific entity; parent is the broader/general entity\n"
            "- Only output pairs where BOTH entities appear in the context\n"
            "- When using provided entities in the output, keep their names exactly as given\n"
            "- Do not output duplicate pairs\n"
            "- A child can have multiple parents at different levels\n"
            '- Examples: "动物" (parent) and "狗" (child); "编程语言" (parent) and "Python" (child).\n'
            "- Do not output any explanatory text; only the relationship list"
        )
        try:
            is_ark_api = "ark.cn-beijing.volces.com" in str(client.base_url) if hasattr(client, 'base_url') else False
                ...
                if ',' in line:
                    ...
                    if len(parts) == 2 and parts[0] and parts[1]:
                        ...
                        if child and parent and child != parent:
                            child_pat = r'\b' + re.escape(child.lower()) + r'\b'
                            parent_pat = r'\b' + re.escape(parent.lower()) + r'\b'
                            if re.search(child_pat, ctx_lower) and re.search(parent_pat, ctx_lower):
                                if (parent, child) not in rels:
                                    rels.add((child, parent))
            return rels
        
        except Exception as e:
            print(f"  ⚠ LLM处理chunk失败: {e}")
            return set()
    
    should_stop = False
    for chunk_idx, chunk in enumerate(chunks):
        if (chunk_idx + 1) % 1 == 0:
            print(f"\r  [正在扫描] Chunk: {chunk_idx + 1} | 已找到关系: {len(relations)}", end="")
        if should_stop:
            break
        
        chunk_entities = find_entities_in_text(chunk, entities)
        if len(chunk_entities) >= 1:
            batch_relations = process_chunk_entities_with_llm(chunk_entities, chunk)
            if batch_relations:
