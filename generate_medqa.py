...
def main(input_file, output_file, max_chunks=None):
    chunks = []
    if input_file.lower().endswith(".txt"):
        with open(input_file, 'r', encoding='utf-8') as f:
            content = f.read()
        content = content.replace("\r\n", "\n").replace("\r", "\n")
        parts = re.split(r"^\s*#\s*Chunk\s+\d+\s*$", content, flags=re.MULTILINE)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Extract relations from MedQA chunks")
    parser.add_argument("--input", type=str, default="./datasets/medqa_chunks.txt")
    parser.add_argument("--output", type=str, default="./entities_new/medqa_relations.csv")
    parser.add_argument("--max-chunks", type=int, default=None)
    args = parser.parse_args()
    main(args.input, args.output, args.max_chunks)
